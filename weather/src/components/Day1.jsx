...
const Day1 = () => {
  ...
  console.log(x)
  const time = x.daily[0].dt;
  const milisegundos = new Date(time * 1000);

  const date = new Date(milisegundos).toDateString("en-GB");
  const t = Math.round(x.daily[0].temp.min - 273.15);
  const t2 = Math.round(x.daily[0].temp.max - 273.15);

  setValue(x.daily[0].weather[0].main);
  if (x.daily[0].weather[0].main === 'Rain') {
    setPic('rainy.png')
  }
  if (x.daily[0].weather[0].main === 'Clouds') {
    setPic('clouds.png')
  }
  if (x.daily[0].weather[0].main === 'clear') {
    setPic('sunny.png')
  }
  setDate(date);
  setTemp(t);
