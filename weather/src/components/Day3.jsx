...
const Day3 = () => {
  ...
  console.log(x)
  const time = x.daily[2].dt;
  const milisegundos = new Date(time * 1000);
  const date = new Date(milisegundos).toDateString("en-GB");
  const t = Math.round(x.daily[2].temp.min - 273.15) 
  const t2 = Math.round(x.daily[2].temp.max - 273.15) 
  setValue(x.daily[2].weather[0].main); 
  if (x.daily[2].weather[0].main === 'Rain') {
    setPic('rainy.png')
  }
  if (x.daily[2].weather[0].main === 'Clouds') {
    setPic('clouds.png')
  }
  if (x.daily[2].weather[0].main === 'clear') {
    setPic('sunny.png')
  }
